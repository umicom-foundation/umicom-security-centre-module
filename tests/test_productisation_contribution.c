...
int main(void)
{
    ...
    assert(strcmp(adoption->application_id, "org.umicom.security-centre") == 0);
    assert(umi_product_application_adoption_validate(adoption) ==
           UMI_STATUS_OK);
    assert(umi_security_centre_productisation_snapshot(&snapshot) ==
           UMI_STATUS_OK);
    ...
    assert(umi_security_centre_product_session_init(&session) ==
           UMI_STATUS_OK);
